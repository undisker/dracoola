...
const
  ...
  { Controls writing of BigTIFF files. Default is IfNeeded - not much software
    can open BigTIFF, so use it only when needed. Values:
    - IfNeeded(0) writes BigTIFF only when the file size for sure exceeds
      the classic TIFF 4 GiB limit (can only be assured for uncompressed TIFFs).
    - IfSafer(1) also uses BigTIFF when compression makes the final size uncertain.
